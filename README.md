...
原来的沉浸式代码太多太乱，而且仍然存在navigationBar无法透下去，连不上效果。使用EdgeToEdge来简化逻辑。

原来代码中一堆动态修改布局，控件的布局规则参数，导致在某些手机上无法正确显示，出现黑屏。
这也会移除不少原来支持的动态化设置控件位置的逻辑和参数。同时，移除了外部设置preview fragment的可能。不再允许从外部设置预览。
